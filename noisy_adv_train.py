...
parser.add_argument("--loss", type=str, choices=["nr", "nas"],
                    default="nr")
...
train_loader_p, _ = load_dataset(args)
...
def main(args):
    ...
    if args.loss == "nr":
        loss_func = NoisyRobustLoss(args)
    elif args.loss == "nas":
        loss_func = NoisyAccurateStableLoss(args)
    else:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        model.train()
        for batch_idx, ((x, y), (x2, y2)) in enumerate(
                zip(train_loader, train_loader_p)):
            x, y = x.to(device), y.to(device)
            x2, y2 = x2.to(device), y2.to(device)

            optimizer.zero_grad()

            loss, ret = loss_func(model, x, y, x2=x2, y2=y2)
